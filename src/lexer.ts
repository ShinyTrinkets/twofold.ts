...
export default class Lexer {
  ...
  push(text: string): void {
    ...
    const commitAndTransition = function (
      newState: string,
      ...
      if (self.pendingState.rawText) {
        if (
          joinState && self.state !== STATE_RAW_TEXT &&
          newState === STATE_RAW_TEXT
        ) {
    ...
    for (const char of text) {
      ...
      if (this.state === STATE_RAW_TEXT) {
        ...
        console.error(
          `Char: ${char}; State: ${this.state}; PriorState: ${this.priorState}`,
        );
        commitAndTransition(STATE_RAW_TEXT, true);
      }
    }
